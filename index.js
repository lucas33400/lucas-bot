...
client.on('guildMemberAdd', member => {
    member.guild.channels.cache.get(config.greeting.channel).send(`${member}`, new Discord.MessageEmbed()
        ...
        Nous te souhaitons donc la **_BIENVENUE_**   Nous sommes désormais ${member.guild.memberCount} ! 🎉`)
        .setColor('#00ff00'))
    console.log(`${member}`, `${member.user.tag}`, 'est arrivé sur le serveur, nous sommes désormais', `${member.guild.memberCount}`)
})
...
client.on('messageReactionAdd', (reaction, user) => {
    ...
    console.log('viens d\'accepter le règlement.')
})
...
client.on('messageReactionRemove', (reaction, user) => {
    ...
    console.log('A tenté de supprimer son rôle.')
})
